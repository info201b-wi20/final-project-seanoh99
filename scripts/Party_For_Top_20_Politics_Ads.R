...
Party_Top_20_Politics_Ad <- function(df) {
  top20 <- df%>%group_by(title) %>% summarize(count =n()) %>% arrange(-count) %>% slice(1:20)
  ...
  Party <- 
    data.frame(Parties = c("Democrats", "Republicans (Donald Trump)", "Non Profit", "Other"), 
                      Count =c(democrats,republicans, Non_Profit, Other))
